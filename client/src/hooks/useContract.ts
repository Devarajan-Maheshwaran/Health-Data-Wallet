...
export const useContract = () => {
  ...
  // Add health record
  const addRecord = useCallback(async (recordType: string, title: string, ipfsHash: string) => {
    if (!contract) {
      throw new Error('Contract not initialized');
    }
  ...
  // Get record count
  const getRecordCount = useCallback(async (patientAddress: string) => {
    if (!contract) {
      throw new Error('Contract not initialized');
    }
  ...
  // Get record details
  const getRecord = useCallback(async (patientAddress: string, recordId: number) => {
    if (!contract) {
      throw new Error('Contract not initialized');
    }
  ...
  const isPatientRegistered = useCallback(async (patientAddress: string) => {
    if (!contract) {
      throw new Error('Contract not initialized');
    }
